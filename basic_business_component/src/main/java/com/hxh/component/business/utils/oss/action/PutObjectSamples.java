...
private CallBackBody callbackbody;
    ...
    public PutObjectSamples(OSS client, String testBucket, String testObject, String uploadFilePath, CallBackBody body) {
        this.oss = client;
    ...
    // 从本地文件上传，使用非阻塞的异步接口
    public void asyncPutObjectFromLocalFile(final RequestCallBack callback) {
        // 构造上传请求
        PutObjectRequest put = new PutObjectRequest(testBucket, testObject, uploadFilePath);
        setCallBack(put);
        // 异步上传时可以设置进度回调
        ...
        head.setContentLength(new File(uploadFilePath).length());
        head.setContentType("application/x-png");

        put.setMetadata(head);
     ...
     * @return
     */
    private PutObjectRequest setCallBack(PutObjectRequest request)
    {
        ...
        callbackVars.put("x:access_token", callbackbody.callbackToken);
